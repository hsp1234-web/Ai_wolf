...
def fetch_fred_data(series_ids_str: str, start_date_str: str, end_date_str: str, api_key: str):
    ...
    try:
        ...
        msg = f"FRED 錯誤：API 金鑰初始化失敗: {str(e)}。" # 移除"請檢查您的 API 金鑰"以避免冗餘
        errors.append(msg)
        logger.error(msg, exc_info=True)
        return data_series_dict, errors
...
@st.cache_data(ttl=86400) # 每日快取 NY Fed 數據
def fetch_ny_fed_data():
        ...
        try:
            ...
            if file_type == "xml":
                try:
                    ...
                    # 根據實際 XML 結構調整 xpath 和 namespaces
                    # 示例 XPath 和 namespace，可能需要根據實際 XML 調整
                    # xpath_query = ".//data:Series"
                    # ns = {"data": "http://www.newyorkfed.org/xml/schemas/markets/soma/soma_2_0_0.xsd"}
                    # df_list = pd.read_xml(io.BytesIO(response.content), parser='lxml', xpath=xpath_query, namespaces=ns)
                    df_list = pd.read_xml(io.BytesIO(response.content), parser='lxml') # 簡化版，如果XML可以直接轉為table
                    ...
                    # --- XML 數據轉換 ---
                    # 這裡的轉換邏輯高度依賴於XML的實際結構
                    # 以下是一個非常假設性的例子，需要替換為針對性的代碼
    ...
    try:
        ...
        # 按日期分組，對於同一日期，如果有多個來源，取最後一個非空值
        final_series = final_series.groupby(final_series.index).last()
        final_series = final_series.sort_index() # 按日期排序
        final_series = final_series.ffill() # 向前填充缺失值
...
if __name__ == "__main__":
    ...
    # logger.info("--- 測試 FRED ---")
    # fred_data, fred_errs = fetch_fred_data("GDP,CPIAUCSL", "20220101", "20230101", st.session_state.fred_api_key)
